...
var _ = Describe("PlacementAPI controller", func() {
	...
	When("A PlacementAPI is created with TLS config", func() {
		BeforeEach(func() {
			DeferCleanup(k8sClient.Delete, ctx, CreatePlacementAPISecret(namespace, SecretName))
			DeferCleanup(keystone.DeleteKeystoneAPI, keystone.CreateKeystoneAPI(namespace))
		...
		It("creates TLS volumes and volume mounts", func() {
			deployment := th.GetDeployment(names.DeploymentName)
	...
	// 	BeforeEach(func() {
	// 		DeferCleanup(k8sClient.Delete, ctx, CreatePlacementAPISecret(namespace, SecretName), CreatePlacementAPISecret(namespace, "ca-certs"), CreatePlacementTLSSecret(namespace, "tls-certs"))
	// 		DeferCleanup(th.DeleteInstance, CreatePlacementAPI(names.PlacementAPIName, GetDefaultPlacementAPISpec()))
	// 		DeferCleanup(keystone.DeleteKeystoneAPI, keystone.CreateKeystoneAPI(namespace))
